...
async function buildFlatGalleryRecord(sourceRoot, galleryDirectoryName) {
...
async function buildNestedGalleryRecord(sourceRoot, artistDirectoryName, galleryDirectoryName) {
...
async function readFlatGalleryDirectories(sourceRoot, layoutHints) {
  ...
  for (const entry of layoutHints.filter((item) => item.hasImages)) {
    ...
    if (!cachedEntry || cachedEntry.directoryMtimeMs !== directoryStat.mtimeMs) {
      gallery = await buildFlatGalleryRecord(sourceRoot, entry.directoryName);
    }
  ...
  return galleries.sort((left, right) => right.updatedAt - left.updatedAt);
}
...
async function readNestedGalleryDirectories(sourceRoot, rootDirectories) {
  ...
  return allGalleries.sort((left, right) => right.updatedAt - left.updatedAt);
}
...
async function loadGalleries() {
  ...
  galleryCachePromise = (async () => {
    let nextCache = {
      ...
      layout: galleryCache.layout,
      flatCache: galleryCache.flatCache,
      artistCaches: galleryCache.artistCaches,
      items: galleryCache.items,
    };
...
export async function getArtistSummaries() {
  ...
  return Array.from(artists.values()).sort((left, right) => right.updatedAt - left.updatedAt);
}
