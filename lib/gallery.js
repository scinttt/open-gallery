...
function getTrashRoot(sourceRoot) {
  return (
    process.env.GALLERY_TRASH_DIR ||
...
export async function moveGalleryToTrashBySlug(slug) {
  ...
  if (!isPathInsideDirectory(sourceRoot, gallery.sourcePath)) {
    const error = new Error("Gallery path is outside the configured source root.");
    error.status = 400;
    throw error;
  }

  const trashRoot = getTrashRoot(sourceRoot);
